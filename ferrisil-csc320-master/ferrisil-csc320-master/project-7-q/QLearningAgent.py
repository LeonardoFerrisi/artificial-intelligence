...
class QLearningAgent:
    ...
    def __init__(self, alpha, gamma, epsilon, maxSteps, maxEp, env: Env):
    ...
    def learn(self):
        ...
        # Generate policy 
        policy = self.generatePolicy(self.explorationRate)
        ...
        for episode in range(self.maxEpisodes):            
            ...
            for i in range(self.maxStepsPerEpisode):
                ...
                if (done):
                    ...
                    # self.printQTable()
            rewards.append(currentReward) #append the average reward
            print(f"Completed Episode {episode}, Average reward = {currentReward/stepsTaken}, currentReward: {currentReward}")
    ...
    def generatePolicy(self, epsilon):
        """
        ...
        def policyFunc(state):
            ...
            action_prob = np.ones(self.actionSize, dtype=float)* self.explorationRate / self.actionSize

            bestAction = np.argmax(self.Qtable[state])

            action_prob[bestAction] += (1.0 - epsilon)
            return action_prob
...
if __name__ == "__main__":
    ...
    q = QLearningAgent(alpha=0.1, gamma=0.6, epsilon=0.5, maxSteps=20, maxEp=1000, env=env)
    print("\n\nEmptyTable:")
    q.printQTable()
    print("================\n\n")
