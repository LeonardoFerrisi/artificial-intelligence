...
class improvedSearch:
    ...
    def __init__(self, initialState, goalState, verbose=False):
        ...
        if solution == None:
            ...
            # print(f"Time Elapsed: {round(timeElapsed, 8)}")
    ...
    def execute(self):
        while not self.q.empty():
            ...
            if current.state.dictkey() not in self.memory.keys():
                self.memory[current.state.dictkey()] = set()
            ...
            if self.goalState.equals(current.state):
                ...
                for nextState in successors:
                    ...
                    if n.state.dictkey() not in self.memory[current.state.dictkey()]:
                        self.memory[current.state.dictkey()].add(
                            n.state.dictkey())
                        self.q.enqueue(n)
                ...
                if self.verbose:
                    ...
                    print(f"Current Memory: {str(self.memory)}")
                    print("-------------------------------")
